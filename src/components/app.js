...
const App = () => {
    ...
    return (
        <ThemeProvider theme={getMuiTheme(theme)}>
            <React.Fragment>
                <CssBaseline />
                <TopBar appName="XXX" appColor="#d36d16" onParametersClick={() => console.log("onParametersClick")} onLogoutClick={() => logout(dispatch, userManager.instance)} onLogoClick={() => onLogoClicked()} user={user}/>
                { user !== null ? (
                        <Switch>
                            <Route exact path="/">
                                <Box mt={20}>
